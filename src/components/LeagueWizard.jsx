...
function Step1({ d, set }) {
  return <>
    ...
    <div style={S.row}>
      ...
      <div style={S.half}>
        <label style={S.lbl}>Durée de la saison</label>
        <select style={S.sel} value={d.duration} onChange={e => set('duration', e.target.value)}>
          ...
          <option value="playoffs">Séries seulement</option>
        </select>
      </div>
    </div>
    <div style={S.mb}>
...
// ---- Étape 2: Capital et mise ----
function Step2({ d, set }) {
  return <>
    <div style={S.info}>Définissez la mise réelle et le capital virtuel de départ de chaque joueur.</div>
    <div style={S.row}>
      <div style={S.half}>
        <label style={S.lbl}>Mise réelle ($ CAD)</label>
    ...
    <div style={S.mb}>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
        {[
          { key:'dividendsEnabled', lbl:'Dividendes activés (victoires = revenus)' },
          { key:'limitOrdersEnabled', lbl:'Ordres limités permis' },
          { key:'elimPenalty', lbl:'Pénalité si équipe éliminée des séries' },
        ].map(opt => (
...
// ---- Étape 3: Distribution des prix ----
function Step3({ d, set }) {
  return <>
    <div style={S.info}>Comment les mises réelles seront-elles distribuées aux gagnants?</div>
    <div style={S.mb}>
    ...
    <div style={S.mb}>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
        {[
          ...
          { key:'bonusMid', lbl:'Bonus à mi-saison' },
        ].map(opt => (
          <label key={opt.key} style={{ display:'flex', alignItems:'center', gap:10, fontSize:14, cursor:'pointer' }}>
...
function Step5({ d, result }) {
  ...
  return <>
    <div style={{ fontSize:15, fontWeight:600, color:'#222', marginBottom:16 }}>Résumé de votre ligue</div>
    {[
      ['Nom', d.name],
      ['Joueurs', `${d.players} joueurs`],
      ['Durée', { week:'1 semaine', month:'1 mois', season:'Saison complète', playoffs:'Séries seulement' }[d.duration]],
      ['Draft', { libre:'Libre', serpentin:'Serpentin' }[d.draft]],
      ['Mise réelle', `${d.mise}$ CAD`],
      ['Capital virtuel', `${d.capital.toLocaleString()}$`],
      ['Distribution', { winner:'Gagnant tout', top2:'Top 2', top3:'Top 3' }[d.prizeMode] || d.prizeMode],
      ['Transactions', d.tradeLimit === 0 ? 'Illimitées' : `${d.tradeLimit}/semaine`],
      ['Invités', (d.emails || []).length > 0 ? (d.emails || []).join(', ') : 'Aucun (code manuel)'],
    ].map(([lbl, val]) => (
        ...
        <div style={S.hdr}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
            <div style={{ fontSize:19, fontWeight:700, color:'#111' }}>
              {result ? 'Ligue créée!' : 'Créer une ligue'}
            </div>
